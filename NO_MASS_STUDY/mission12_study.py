"""
MISSION-12 STUDY — feature search and time plots on the +/-12 s mission window.

STANDALONE. Reads only flights/F*/flight_resampled.csv, writes only into this folder.

THE WINDOW
----------
Every run uses the flight plus 12 s either side. 12 s is the smallest fixed margin
that captures motor ARMING on all 14 flights (largest arm-to-takeoff gap is 11.9 s).
It is a physical criterion, not a tuned constant: a margin sweep shows cruise error
is flat at 34-35 W for every window from 0 to 30 s, so the choice affects scope, not
performance. Beyond 12 s the extra rows are idle and add nothing.

The power-ON event is not in this dataset — every recording begins with the aircraft
already energised at ~25-27 W — so the window cannot start earlier than that.
...
Run:
    python mission12_study.py
    python mission12_study.py --with-mass
    python mission12_study.py --rate 20
    python mission12_study.py --rate 20 --with-mass
"""
...
OUT = os.path.join(HERE, "MISSION12")
...
# ---- Hyperparameters ----------------------------------------------------------
# PARAMS holds the XGBoost hyperparameters used everywhere in this study. It starts
# at the previous fixed configuration and is REPLACED in main() by the result of a
# RandomizedSearchCV (GroupKFold by flight) before any feature selection is run, so
# the greedy search, the top-K plots and the final metrics all use the tuned model.
# Trees are scale-invariant, so tuning is done on the raw features (no leakage from
# standardisation); the leave-one-flight-out evaluation still standardises per fold.
PARAMS = dict(n_estimators=150, max_depth=5, learning_rate=0.1, subsample=0.9)

N_ITER = 30        # RandomizedSearchCV candidates (matches 06_train_xgboost.py)
PARAM_DIST = {
...
def tune(data, cols):
    """
    Select XGBoost hyperparameters by RandomizedSearchCV with GroupKFold on the
    flight id, so no flight is split across a search fold. Scores negative MAE, the
    same metric the study reports. Returns (best_params, best_cv_mae).

    Trees are invariant to feature scaling, so the search runs on the raw features;
    this avoids any leakage that standardising on the full dataset would introduce.
    """
    X = data[cols].values
    y = data["power"].values
    groups = data["flight_id"].values
    n = data["flight_id"].nunique()
    ...
    search = RandomizedSearchCV(
        ...
        scoring="neg_mean_absolute_error", cv=GroupKFold(n_splits=n),
...
def evaluate(data, cols):
    ...
    for h in sorted(data["flight_id"].unique()):
        ...
        m = XGBRegressor(objective=OBJ, n_jobs=-1, random_state=42, **PARAMS)
...
def fig_time(data, preds):
    ...
    for r, fid in enumerate(show):
        ...
        for j, (label, p_all) in enumerate(preds.items()):
            p = p_all[k]
            # R2 scored WITHIN this flight — i.e. against this flight's own mean.
            # Much harsher than the pooled R2 in the other figures, and the right
            # question for a single-flight panel: does the line track this trace?
    ...
    fig.suptitle(f"Mission window ±{MARGIN_S} s — {RATE} Hz, "
                 f"{'mass included' if WITH_MASS else 'mass withheld'}\n"
...
def main():
    ...
    global PARAMS
    print(f"  tuning hyperparameters: RandomizedSearchCV, {N_ITER} candidates, "
          f"GroupKFold by flight, scoring MAE...", flush=True)
    best_params, best_cv_mae = tune(data, cols_for(list(GROUPS)))
    PARAMS = best_params
    cv_mae = best_cv_mae
    print(f"  best CV MAE = {cv_mae:.2f} W")
    ...
    # other_mae_w is None if the window contains no non-cruise rows (MARGIN_S = 0)
    ...
    with open(os.path.join(OUT, f"summary_{TAG}.json"), "w") as fh:
        json.dump({"rate_hz": RATE, "with_mass": WITH_MASS, "margin_s": MARGIN_S,
                   ...
                   "hyperparameter_search": {
                       ...
                       "n_candidates": N_ITER,
                       "cv": "GroupKFold by flight",
                       "scoring": "neg_mean_absolute_error",
                       "best_cv_mae_w": round(cv_mae, 2)},
    ...
    print(f"  Saved → MISSION12/{os.path.basename(p1)}")
    print(f"  Saved → MISSION12/{os.path.basename(p2)}")
    print(f"  Saved → MISSION12/{os.path.basename(p3)}")
    print(f"  Saved → MISSION12/{os.path.basename(p4)}")
    print(f"  Saved → MISSION12/search_{TAG}.csv, summary_{TAG}.json, "
