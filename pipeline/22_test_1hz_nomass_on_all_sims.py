"""
Step 22 – Apply the 1 Hz NO-MASS row-level model to every SIM flight.

Uses only the 1 Hz no-mass winner from NO_MASS_STUDY/MISSION12
(summary_1hz_nomass.json): motors + velocity  (the best-scoring config overall).
Same recipe as steps 18/21: XGBoost reg:absoluteerror, ±12 s window, per-fold
standardization, leave-one-flight-out, everything aggregated to 1 Hz. Fold models
are cached and reused across SIM flights.

Input : flights/F*/flight_resampled.csv
        SIM_FLIGHTS/<variant>/flight_resampled.csv (+ segment_map.csv)
Output : SIM_FLIGHTS/<variant>/1hz/nomass_1hz.png
         SIM_FLIGHTS/<variant>/1hz/nomass_1hz.csv

Run: python 22_test_1hz_nomass_on_all_sims.py
"""
...
def run_variant(train, variant):
    ...
    ax.plot(t, p, color=COLOR, lw=1.5, marker="o", ms=2, alpha=0.9,
            label=f"no-mass 1 Hz (motors + velocity)   MAE {mae:.0f} W (R² {r2:.2f})")
    ...
    ax.set_title(f"{variant} — 1 Hz no-mass row-level model\n"
    ...
    ax.grid(alpha=0.3)
    plt.tight_layout()
    png = os.path.join(out_dir, "nomass_1hz.png")
    plt.savefig(png, dpi=150, bbox_inches="tight"); plt.close()

    pd.DataFrame({"second": t, "source_flight": mission["_src"].values,
                  "actual_power": y.round(1), "pred_nomass": p.round(1)}
                 ).to_csv(os.path.join(out_dir, "nomass_1hz.csv"), index=False)

    print(f"  {variant:16s}  MAE {mae:5.1f} W   R² {r2:.3f}   -> 1hz/{os.path.basename(png)}")


def main():
    print("\n[Step 22] 1 Hz no-mass model on all SIM flights")
